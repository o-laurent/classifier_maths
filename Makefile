...
batch_lin_cl: src/fmatrix.cu src/cuda_stuff.cu src/stable_softmax.cu src/read_csv.cu src/evaluate_accuracy.cu src/classifier_math.cu src/normalization.cu src/linear_classification_batch.cu src/preprocess_data.cu
	nvcc -arch=sm_86 -g -G -lcublas -lcusolver src/linear_classification_batch.cu src/normalization.cu src/stable_softmax.cu src/read_csv.cu src/preprocess_data.cu src/evaluate_accuracy.cu src/fmatrix.cu src/classifier_math.cu src/cuda_stuff.cu -o linear_classification
